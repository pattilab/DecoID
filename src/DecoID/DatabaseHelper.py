...
class mzCloudpy():
    ...
    def getTreesAsync(trees, calibration="recalibrated", maxRetries=3, cache="none", maxMass=MAXMASS, resolution=2):
        ...
        output = {key: getAllSpectraInTree(reformatSpectraDictList(json.loads(val.text)), maxMass, resolution) for
    ...
    def getListofSpectrainTree(trees, calibration="recalibrated", library="reference"):
        ...
        while (len(errors) > 0):
            ...
            for tree in [treeOrder[x] for x in errors]:
                ...
                payload = ""
                requestTuple.append(
                    grequests.get(url, data=payload, headers=HEADERSSPEC, params=querystring, timeout=40))
            requestTuple = tuple(requestTuple)
        ...
        output = {tree: reformatSpectraDictList(json.loads(response.text)) for response, tree in
    ...
    def generateCompoundID2SpectralIDIndexedByM_ZStrict(numPerPage=100, library="reference"):
        ...
        totalCompounds = json.loads(grequests.map((grequests.get(COMPOUNDURL.replace("LIBRARY", library), data="",
                                                                 headers=HEADERSCOMP, params={"page": str(1),
                                                                                              "pageSize": "5",
                                                                                              "newer": "2000-09-03"}),))[
                                        0].text)["Total"]
        ...
        for page in range(1, numPages + 1):
            ...
            for comp in compounds:
                treeDict.update({(key, comp, compounds[comp]["SearchCompoundName"]): val for key, val in
                                 reformatSpectraDictList(compounds[comp]["SpectralTrees"]).items()})
